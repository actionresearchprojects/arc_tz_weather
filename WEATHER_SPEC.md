...
- **Wind data quality**: Raw data contains erroneous spikes on both channels (e.g. avg values jumping to 70+ km/h in otherwise calm periods; peak values of 431, 611, 803 km/h from reed switch bounce). Four QC filters are applied via `wind_qc()` in `modules/common.py`: (1) avg rolling-median ratio spike filter, (2) peak/avg bounce ratio filter, (3) hard ceilings (avg > 60 km/h, peak > 100 km/h), (4) physical impossibility — when avg > peak both channels are NaN'd. See `WIND_QC.md` for full specification.
